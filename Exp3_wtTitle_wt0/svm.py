...
def extract_feature(data_file, save_vocab):
    ...
    text_data = data_df['sentence_text']
    ...
    if save_vocab:
        vocab_file = "results/Exp3_wtTitle_wt0/svm/models/vectorizer.pkl"
...
def evaluate_model_with_cross_validation(X, y, data_df, mlb_labels, n_splits=5):
    ...
    freq_df.to_excel("results/Exp3_wtTitle_wt0/svm/mismatches_frequency.xlsx", index=False)
    # Sauvegarde des prédictions dans un fichier Excel
    val_df = pd.DataFrame(excel_results)
    val_df.to_excel("results/Exp3_wtTitle_wt0/svm/predictions_original_values.xlsx", index=False)
...
if __name__ == "__main__":
    train_data_file = "data/Methods_all_wt0.xlsx"
    ...
    model_filename = 'results/Exp3_wtTitle_wt0/svm/models/final_model.sav'
    ...
    val_df = pd.DataFrame(excel_results)
    val_df.to_excel("results/Exp3_wtTitle_wt0/svm/predictions_with_probabilities.xlsx", index=False)