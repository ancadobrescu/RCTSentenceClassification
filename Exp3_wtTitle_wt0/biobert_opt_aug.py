...
parser.add_argument('--data_path', help='path to .csv consisting of sentences', default="data/Dataset_aug_wt0.xlsx")
parser.add_argument('--model_path', help='path to directory consisting of BERT model', default="dmis-lab/biobert-base-cased-v1.2")
parser.add_argument('--model_save_path', help='path to directory consisting of BERT model', default="results/Exp3_wtTitle_wt0/biobert_opt_aug/models")
...
for seed in seeds:
    ...
    for train_index, val_index in skf.split(np.zeros(len(y_other)), y_other):
        ...
        with pd.ExcelWriter(f"results/Exp3_wtTitle_wt0/biobert_opt_aug/train_fold_{fold_no}_seed_{seed}.xlsx") as writer:
            train_df.to_excel(writer, index=False, sheet_name="Train Data")

        with pd.ExcelWriter(f"results/Exp3_wtTitle_wt0/biobert_opt_aug/test_fold_{fold_no}_seed_{seed}.xlsx") as writer:
...
seed_results_df.to_excel("results/Exp3_wtTitle_wt0/biobert_opt_aug/seed_results.xlsx", index=False)

# Fusionner toutes les prédictions en un seul fichier Excel
final_predictions = pd.concat(all_predictions, ignore_index=True)
final_predictions.to_excel("results/Exp3_wtTitle_wt0/biobert_opt_aug/all_predictions.xlsx", index=False)
...
metrics_df.to_excel("results/Exp3_wtTitle_wt0/biobert_opt_aug/cross_validation_metrics.xlsx")
...
final_metrics_df.to_excel("results/Exp3_wtTitle_wt0/biobert_opt_aug/final_metrics.xlsx", index=False)
...
plt.ylabel('F1 Score')
plt.grid(True)
plt.savefig("results/Exp3_wtTitle_wt0/biobert_opt_aug/f1_variability_seeds.png")
plt.show()