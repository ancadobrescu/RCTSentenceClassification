...
parser.add_argument('--data_path', help='path to .csv consisting of sentences', default="data/Methods_all_wt0.xlsx")
parser.add_argument('--model_path', help='path to directory consisting of BERT model', default="dmis-lab/biobert-base-cased-v1.2")
parser.add_argument('--model_save_path', help='path to directory consisting of BERT model', default="results/Exp2_wt0/biobert_opt/models")
...
for seed in seeds:
    ...
    for train_index, val_index in skf.split(np.zeros(len(y)), y):
        ...
        with pd.ExcelWriter(f"results/Exp2_wt0/biobert_opt/train_fold_{fold_no}_seed_{seed}.xlsx") as writer:
            train_df.to_excel(writer, index=False, sheet_name="Train Data")

        with pd.ExcelWriter(f"results/Exp2_wt0/biobert_opt/test_fold_{fold_no}_seed_{seed}.xlsx") as writer:
...
seed_results_df.to_excel("results/Exp2_wt0/biobert_opt/seed_results.xlsx", index=False)

# Fusionner toutes les prédictions en un seul fichier Excel
final_predictions = pd.concat(all_predictions, ignore_index=True)
final_predictions.to_excel("results/Exp2_wt0/biobert_opt/all_predictions.xlsx", index=False)
...
metrics_df.to_excel("results/Exp2_wt0/biobert_opt/metrics.xlsx")
...
final_metrics_df.to_excel("results/Exp2_wt0/biobert_opt/final_metrics.xlsx", index=False)
...
plt.ylabel('F1 Score')
plt.grid(True)
plt.savefig("results/Exp2_wt0/biobert_opt/f1_variability_seeds.png")
plt.show()