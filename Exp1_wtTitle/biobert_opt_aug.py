...
parser.add_argument('--data_path', help='path to .xlsx consisting of sentences', default="data/Dataset_aug.xlsx")
parser.add_argument('--model_path', help='path to directory consisting of BERT model', default="dmis-lab/biobert-base-cased-v1.2")
parser.add_argument('--model_save_path', help='path to directory consisting of BERT model', default="results/Exp1_wtTitle/biobert_opt_aug/models")
...
for seed in seeds:
    ...
    for train_index, val_index in skf.split(np.zeros(len(y_other)), y_other):
        ...
        with pd.ExcelWriter(f"results/Exp1_wtTitle/biobert_opt_aug/train_fold_{fold_no}_seed_{seed}.xlsx") as writer:
            train_df.to_excel(writer, index=False, sheet_name="Train Data")

        with pd.ExcelWriter(f"results/Exp1_wtTitle/biobert_opt_aug/test_fold_{fold_no}_seed_{seed}.xlsx") as writer:
        ...
        results_df['fold'] = fold_no  # Numéro du pli
        ...
        prob_df = prob_df[consort_items]  # Ceci garantira que les colonnes sont dans l'ordre voulu
        ...
        for i, class_name in enumerate(consort_items):
            ...
            if len(np.unique(y_true[:, i])) > 1 and len(np.unique(y_pred[:, i])) > 1:
                ...
                roc_auc_class = None  # Aucun ROC AUC calculé si une seule classe est présente
...
seed_results_df.to_excel("results/Exp1_wtTitle/biobert_opt_aug/seed_results.xlsx", index=False)

# Fusionner toutes les prédictions en un seul fichier Excel
final_predictions = pd.concat(all_predictions, ignore_index=True)
final_predictions.to_excel("results/Exp1_wtTitle/biobert_opt_aug/all_predictions.xlsx", index=False)
...
metrics_df.to_excel("results/Exp1_wtTitle/biobert_opt_aug/cross_validation_metrics.xlsx")
...
final_metrics_df.to_excel("results/Exp1_wtTitle/biobert_opt_aug/final_metrics.xlsx", index=False)
...
plt.ylabel('F1 Score')
plt.grid(True)
plt.savefig("results/Exp1_wtTitle/biobert_opt_aug/f1_variability_seeds.png")
plt.show()