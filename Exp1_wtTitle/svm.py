...
def extract_feature(data_file, save_vocab):
    ...
    if save_vocab:
        vocab_file = "results/Exp1_wtTitle/svm/models/vectorizer.pkl"
...
def evaluate_model_with_cross_validation(X, y, data_df, mlb_labels, n_splits=5):
    ...
    freq_df.to_excel("results/Exp1_wtTitle/svm/mismatches_frequency.xlsx", index=False)
    # Sauvegarde des prédictions dans un fichier Excel
    val_df = pd.DataFrame(excel_results)
    val_df.to_excel("results/Exp1_wtTitle/svm/predictions_original_values.xlsx", index=False)
...
if __name__ == "__main__":
    ...
    print("Fichier de données : ", args.data_file)
    ...
    model_filename = 'results/Exp1_wtTitle/svm/models/final_model.sav'
    ...
    val_df = pd.DataFrame(excel_results)
    val_df.to_excel("results/Exp1_wtTitle/svm/predictions_with_probabilities.xlsx", index=False)
