...
parser.add_argument('--model_save_path', help='path to directory consisting of BERT model', default="results/MainExp/biobert_opt/models")
...
train_df_linh["text"] = train_df_linh["sentence_text"] + " " + train_df_linh["top_section"]

# Ajouter les colonnes one-hot encodées de top_section comme traits distincts
...
for seed in seeds:
    ...
    for train_index, val_index in skf.split(np.zeros(len(y)), y): 
        ...
        with pd.ExcelWriter(f"results/MainExp/biobert_opt/train_fold_{fold_no}_seed_{seed}.xlsx") as writer:
            train_df.to_excel(writer, index=False, sheet_name="Train Data")
        
        with pd.ExcelWriter(f"results/MainExp/biobert_opt/test_fold_{fold_no}_seed_{seed}.xlsx") as writer:
        ...
        results_df['predicted_labels'] = predicted_classes  # Utiliser les labels réels
        results_df['true_labels'] = true_labels  # Labels réels
...
seed_results_df.to_excel("results/MainExp/biobert_opt/seed_results.xlsx", index=False)

# Fusionner toutes les prédictions en un seul fichier Excel
final_predictions = pd.concat(all_predictions, ignore_index=True)
final_predictions.to_excel("results/MainExp/biobert_opt/all_predictions.xlsx", index=False)
...
metrics_df.to_excel("results/MainExp/biobert_opt/metrics.xlsx")
...
final_metrics_df.to_excel("results/MainExp/biobert_opt/final_metrics.xlsx", index=False)
...
plt.ylabel('F1 Score')
plt.grid(True)
plt.savefig("results/MainExp/biobert_opt/f1_variability_seeds.png")
plt.show()