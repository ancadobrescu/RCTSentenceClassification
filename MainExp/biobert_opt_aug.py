...
parser.add_argument('--model_save_path', help='path to directory consisting of BERT model', default="results/MainExp/biobert_opt_aug/models")
...
try:
    ...
    forced_example_index = None  # Si aucun exemple ne correspond, ne pas forcer d'exemple
...
for seed in seeds:
    ...
    for train_index, val_index in skf.split(np.zeros(len(y_other)), y_other):
        ...
        with pd.ExcelWriter(f"results/MainExp/biobert_opt_aug/train_fold_{fold_no}_seed_{seed}.xlsx") as writer:
            train_df.to_excel(writer, index=False, sheet_name="Train Data")

        with pd.ExcelWriter(f"results/MainExp/biobert_opt_aug/test_fold_{fold_no}_seed_{seed}.xlsx") as writer:
...
seed_results_df.to_excel("results/MainExp/biobert_opt_aug/seed_results.xlsx", index=False)

# Fusionner toutes les prédictions en un seul fichier Excel
final_predictions = pd.concat(all_predictions, ignore_index=True)
final_predictions.to_excel("results/MainExp/biobert_opt_aug/all_predictions.xlsx", index=False)
...
metrics_df.to_excel("results/MainExp/biobert_opt_aug/cross_validation_metrics.xlsx")
...
final_metrics_df.to_excel("results/MainExp/biobert_opt_aug/final_metrics.xlsx", index=False)
...
plt.ylabel('F1 Score')
plt.grid(True)
plt.savefig("results/MainExp/biobert_opt_aug/f1_variability_seeds.png")
plt.show()